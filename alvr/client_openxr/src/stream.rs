        ...
        if !buffer_ptr.is_null() {
            if let Some(xr_now) = crate::xr_runtime_now(self.xr_session.instance()) {
                ...
                if self.core_context.platform().is_yvr() {
                    // YVR runtime reports xr_instance.now() and predicted_display_time on
                    // different time domains. Estimate and cache the offset so tracking
                    // predictions and vsync_queue land on the same clock as the display time.
                    let bias = vsync_time
