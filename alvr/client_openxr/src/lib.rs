...
pub fn entry_point() {
    ...
    // Surface the real device identity in logcat for calibrating the YVR
    // server-side auto-preset against real Build.MANUFACTURER/MODEL values.
    // (The lobby HUD header also shows manufacturer + model persistently, see
    // client_core's set_hud_message.)
    info!(
        "Device info: platform: {platform}, manufacturer: {mfr}, model: {model}, build device: {dev}, build product: {prod}",
        platform = platform,
        mfr = alvr_system_info::device_manufacturer(),
