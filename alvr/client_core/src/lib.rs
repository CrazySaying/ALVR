...
// Converts a canted view to a proportional circumscribed orthogonal view. SteamVR expects
// orthogonal (non-canted) views, while the runtime may hand us canted ones (e.g. YVR/PFDMR).
// The conversion preserves the aspect ratio by scaling the FOV so the original canted corners
// are circumscribed within the orthogonal FOV.
